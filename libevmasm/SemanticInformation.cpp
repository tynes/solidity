...
vector<SemanticInformation::Operation> SemanticInformation::readWriteOperations(Instruction _instruction)
{
	...
	{
		vector<Operation> externalEffects =
			vector<Operation>{Operation{Location::Storage, Effect::Read, {}, {}, {}}} +
			(
				_instruction == Instruction::STATICCALL ?
				vector<Operation>{} :
				vector<Operation>{Operation{Location::Storage, Effect::Write, {}, {}, {}}}
			);
		...
		} + move(externalEffects) + vector<Operation>{
			Operation{
				Location::Memory,
				Effect::Write,
				paramCount - 2,
