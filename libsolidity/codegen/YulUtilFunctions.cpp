...
string YulUtilFunctions::readFromTransientStorage(Type const& _type, size_t _offset, bool _splitFunctionTypes)
{
	if (_type.isValueType())
...
string YulUtilFunctions::readFromTransientStorageDynamic(Type const& _type, bool _splitFunctionTypes)
{
	if (_type.isValueType())
...
string YulUtilFunctions::readFromTransientStorageValueType(Type const& _type, optional<size_t> _offset, bool _splitFunctionTypes)
{
	solAssert(_type.isValueType(), "");
...
string YulUtilFunctions::readFromTransientStorageReferenceType(Type const& _type)
{
	if (auto const* arrayType = dynamic_cast<ArrayType const*>(&_type))
	{
		solAssert(arrayType->dataStoredIn(DataLocation::Memory), "");
	...
	string functionName = "read_from_transient_storage_reference_type_" + _type.identifier();
...
string YulUtilFunctions::updateTransientStorageValueFunction(
	Type const& _fromType,
	Type const& _toType,
	std::optional<unsigned> const& _offset
)
