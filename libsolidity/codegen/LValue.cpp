		...
		else if (m_dataType->category() == Type::Category::Struct)
		{
			...
			else
			{
				for (auto const& member: structType.members(nullptr))
				{
					...
					else if (sourceType.location() == DataLocation::Transient)
					{
						...
						m_context << u256(offsets.second);
						// stack: source_ref target_ref source_member_ref source_member_off
						TransientStorageItem(m_context, *sourceMemberType).retrieveValue(_location, true);
						// stack: source_ref target_ref source_value...
					}
					else
					{
...
{
	solAssert(!_declaration.immutable(), "");
	auto const& location = m_context.storageLocationOfTransientVariable(_declaration);
	m_context << location.first << u256(location.second);
}
...
void TransientStorageItem::storeValue(Type const& _sourceType, SourceLocation const& _location, bool _move) const
{
	CompilerUtils utils(m_context);
	solAssert(m_dataType, "");
		...
		else if (m_dataType->category() == Type::Category::Struct)
		{
			...
			else
			{
				for (auto const& member: structType.members(nullptr))
				{
					...
					m_context << u256(offsets.second);
					// stack: source_ref target_ref target_off source_value... target_member_ref target_member_byte_off
					TransientStorageItem(m_context, *memberType).storeValue(*sourceMemberType, _location, true);
				}
			}
			// stack layout: source_ref target_ref
			solAssert(sourceType.sizeOnStack() == 1, "Unexpected source size.");
...
void TransientStorageItem::setToZero(SourceLocation const&, bool _removeReference) const
{
	...
	{
		// stack layout: storage_key storage_offset
		// @todo this can be improved: use TransientStorageItem for non-value types, and just store 0 in
		// all slots that contain value types later.
		auto const& structType = dynamic_cast<StructType const&>(*m_dataType);
		for (auto const& member: structType.members(nullptr))
		{
			...
			m_context
				<< offsets.first << Instruction::DUP3 << Instruction::ADD
				<< u256(offsets.second);
			TransientStorageItem(m_context, *memberType).setToZero();
		}
		if (_removeReference)
			m_context << Instruction::POP << Instruction::POP;
	}
